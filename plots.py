...
def plot_loss(soc_solver_list, variable='norm_sqd_diff', save_figure=False, file_name=None, plots_folder_name=None, set_ylims=False, ylim_inf=None, ylim_sup=None, title=None, use_fixed_colors=False):
    ...
    for k, soc_solver in enumerate(soc_solver_list):
        ...
        if variable == 'control_objective_mean':
            ...
            iterations_array = np.array(training_info['control_objective_itr'])[first_point_control_loss:]
        ...
        print(f'variable_array.shape: {variable_array.shape}, iterations_array.shape: {iterations_array.shape}')
        print(f'np.mean(variable_array): {np.mean(variable_array)}')
        if variable == 'control_objective_mean':
            plt.plot(iterations_array, variable_array, label=soc_solver.legend_name,
                     color=color, linestyle=lss[k])
        else:
            plt.semilogy(iterations_array, variable_array, label=soc_solver.legend_name,
                         color=color, linestyle=lss[k])
        if variable == 'control_objective_mean':
            std_err_array = torch.stack(training_info['control_objective_std_err']).cpu().numpy()[first_point_control_loss:]
            bar_lower = variable_array - std_err_array
            bar_upper = variable_array + std_err_array
            plt.fill_between(iterations_array, bar_lower, bar_upper, alpha=.3, color=color)
